...
const product = {
  serialNo: 1, articleCode: '534.84.523',
  articleName: 'Teresa Neo I-90 Bldc Hood', category: 'Cooker Hoods',
  mrp: 48120, rrp: 36768, dealerPricePreTax: 11931,
...
describe('ProductList', () => {
  it('renders a list of product cards', () => {
    render(<ProductList products={[product]} quoteItems={[]} onAdd={vi.fn()} isLoading={false} status="ready" />);
    expect(screen.getByText('534.84.523')).toBeInTheDocument();
  });

  it('shows empty state when no products', () => {
    render(<ProductList products={[]} quoteItems={[]} onAdd={vi.fn()} isLoading={false} status="ready" />);
    expect(screen.getByText(/No products found/i)).toBeInTheDocument();
  });

  it('shows loading skeletons when isLoading', () => {
    render(<ProductList products={[]} quoteItems={[]} onAdd={vi.fn()} isLoading={true} status="loading" />);
    expect(screen.queryByText(/No products/i)).not.toBeInTheDocument();
  });

  it('shows error state when status is error', () => {
    render(<ProductList products={[]} quoteItems={[]} onAdd={vi.fn()} isLoading={false} status="error" />);
